...
function Subscription(props) {
            ...
            return (
                <Switch checked={e.isActive} onClick={() => {
                    confirm(confirmMessage(`${e.isActive ? 'de' : ''}active`)).then(async () => {
                        const res = await apiManager.put(`subscription/status/${e._id}`, {
                            status: !e.isActive
                        })
                        if (!res.error) {
    ...
    return (
        ...
        >
            ...
            <SubscriptionAddEdit
                clearSearchField={clearSearchField} setSearch={setSearch} editData={editData} getList={getList} rowsPerPage={rowsPerPage} ref={modalRef} />

            {children}
        </TableHeader>
    );
}
export default withPagination(Subscription, 'subscription/list', { imageRequired: true });